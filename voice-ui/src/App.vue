<template>
  <body>
    ...
    <div v-if="voice.uiEnabled" class="voiceInfo">
      <p
        v-if="voice.callInfo !== 0 && voice.callUiEnabled"
        :class="{ talking: voice.talking }"
      >
        [Call]
      </p>
      <p
        v-if="
          voice.radioEnabled && voice.radioChannel !== 0 && voice.radioUiEnabled
        "
        :class="{ talking: voice.usingRadio }"
      >
        {{ voice.radioChannel }} Mhz [Radio]
      </p>
      <p
        v-if="voice.voiceModes.length && voice.mainUiEnabled"
        :class="{ talking: voice.talking }"
      >
...
export default {
  ...
  setup() {
    ...
    window.addEventListener("message", function (event) {
      ...
      if (data.mainUiEnabled !== undefined) {
        voice.mainUiEnabled = data.mainUiEnabled;
      }

      if (data.callUiEnabled !== undefined) {
        voice.callUiEnabled = data.callUiEnabled;
      }

      if (data.radioUiEnabled !== undefined) {
        voice.radioUiEnabled = data.radioUiEnabled;
      }
      ...
      if (
        data.usingRadio !== undefined &&
        data.usingRadio !== voice.usingRadio
      ) {
